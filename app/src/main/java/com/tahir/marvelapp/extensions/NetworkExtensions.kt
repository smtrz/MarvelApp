package com.tahir.marvelapp.extensions


import com.tahir.marvelapp.generics.ResponseResult
import com.tahir.marvelapp.utils.NetworkHelper
...
fun <T : Any> Flow<ResponseResult<T>>.applyCommonSideEffects() =
	retryWhen { cause, attempt ->
		when {
			(cause is IOException && attempt < NetworkHelper.MAX_RETRIES) -> {
				delay(NetworkHelper.getBackoffDelay(attempt))
				true
			}
			else -> {
				false
